...
void taskUpdateConfigs(void* pvParameters) {
  ...
  // Aguarde um pouco antes de enviar a primeira configuração
  ...
  if (isWiFiConnected && httpSemaphore != NULL) {
    ...
    Serial.println(
        "WiFi não conectado ou HTTP não inicializado. Aguardando...");
  }
  ...
  while (1) {
    if (isWiFiConnected && httpSemaphore != NULL) {
      ...
      if (metadataStr != "{}") {
        ...
        if (metadata != NULL) {
          ...
          cJSON* configurations =
              cJSON_GetObjectItem(metadata, "configurations");
          if (cJSON_IsObject(configurations)) {
            ...
            if (cJSON_IsObject(wifiConfigs)) {
              ...
              // Verifica senha
              cJSON* passwordJson =
                  cJSON_GetObjectItem(wifiConfigs, "password");
            ...
            if (configsChanged || wifiChanged) {
              sendMessageToServer(buildDeviceConfigurationsPayload());
              Serial.println(
                  "Configurações atualizadas e enviadas ao servidor.");
            }
      ...
      Serial.println(
          "Não é possível atualizar configurações: WiFi ou HTTP indisponível");
    }
